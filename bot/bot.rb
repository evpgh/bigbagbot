...
Rubotnik.route :message do
  ...
  bind 'buy', 'bought', 'acquired', 'received', to: :parse_buy_transaction do
    # transaction = persist_transaction(parse_buy_transaction(@message.text))
    transaction = parse_buy_transaction(@message.text)
    transaction.user_id = get_user_info[:id]
    transaction.save
    say "So you acquired #{transaction.quantity} #{transaction.asset.label} for #{transaction.price}?"
  end
